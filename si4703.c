...
void si4703_tune(int fd, u16_t freq)
{
	//int chn = 883;
	//chn *= 10;
	//chn -= 875;
	//chn /= 2;
	u16_t chn = si4703_freq2chan(freq);
...
u16_t si4703_chan2freq(u16_t chan)
{
	//F = S x C + L
	//C = (F - L)/S

	u16_t f = (chan * 0.1) + 875;
...
void si4703_print_meta(int fd)
{
	...
	printf("| Frequency: %3.1f Mhz ", (rchan * 0.1) + 87.5);
	printf("| Channel value: %d ", rchan);
	printf("| RSSI: %d ", rssi & 0x00FF);
	printf("| Mode: %s\n", ((rssi & 0x0100) > 0) ? "Stereo" : "Mono");
	printf("\033[0m");
	printf("RDS Block A %x\n", si4703_regs[0x0C]);
	printf("RDS Block B %x\n", si4703_regs[0x0D]);
	printf("RDS Block C %x\n", si4703_regs[0x0E]);
	printf("RDS Block D %x\n", si4703_regs[0x0F]);
}
...
{
	u8_t buff[16] = {0};
	u8_t device_id = 0, chip_rev = 0, chip_number = 0;
	u16_t manu_id = 0, firmware_ver = 0;
	...
	chip_rev  = (si4703_regs[01] >> 10) & 0x3F;
	chip_number  = (si4703_regs[01] >> 6) & 0x0F;
	firmware_ver = si4703_regs[01] & 0x3F;
